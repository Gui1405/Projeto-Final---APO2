...
<head>
    <meta charset="UTF-8">
    <title>Gerenciar Limpeza - Admin APO2</title>
    ...
    <style>
        ...
        .btn-iniciar { background: linear-gradient(90deg, #10b981, #059669); border: none; color: white; }
        .btn-finalizar { background: linear-gradient(90deg, #ef4444, #b91c1c); border: none; color: white; }
    </style>
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark mb-4">
        <div class="container">
            <a class="navbar-brand fw-bold text-info" href="dashboard.jsp">CINEMA ADMIN - LIMPEZA</a>
            <a href="dashboard.jsp" class="btn btn-outline-light btn-sm">Voltar ao Painel</a>
        </div>
    </nav>

    <div class="container pb-5">
        <h2 class="mb-4 text-center">Status de Limpeza das Salas</h2>
    ...
    <script>
        $(document).ready(function(){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        $.each(data, function(i, s){
                            let statusTexto = s.statusLimpeza || "Desconhecido";
                            let cardBorder = (statusTexto === 'EM ANDAMENTO') ? 'border-warning' : 'border-secondary';
                            ...
                            if(statusTexto !== 'EM ANDAMENTO'){
                                html += '<button class="btn btn-iniciar btn-sm w-100" onclick="alterarLimpeza('+s.salaId+', \'iniciar\')">Iniciar Limpeza</button>';
                            } else {
                                html += '<button class="btn btn-finalizar btn-sm w-100" onclick="alterarLimpeza('+s.salaId+', \'finalizar\')">Finalizar Limpeza</button>';
                            }
            ...
            window.alterarLimpeza = function(salaId, acao) {
                $.ajax({
                    url: '../api/admin/limpeza',
                    type: 'POST',
